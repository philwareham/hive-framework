...
Where the @[task]@ is either @test@, @devupdate@, @build@ or @watch@. Watch will launch a task that watches for file changes; the project is automatically built if a source file is modified. The build task builds the project. The devupdate task checks that development dependencies (Grunt plugins) are the latest versions, and if not, updates them.
